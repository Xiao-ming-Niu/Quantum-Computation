...
\Qcircuit @C=1em @R=.9em {
    \lstick{\ket{0^a}} & \gate{O_{prep}} & \multigate{1}{O_{sel}} & \gate{O_{prep}^\dagger} & \meter & \rstick{0^a?} \qw \\
    \lstick{\ket{\psi}} & \qw & \ghost{O_{sel}} & \qw & \qw & \rstick{\frac{A\ket{\psi}}{\alpha}\ \text{(成功分支)}} \qw
}
\end{equation*}
测量辅助寄存器得到 $\ket{0^a}$ 时, 数据寄存器处于 $A\ket{\psi}/\norm{A\ket{\psi}}$
(概率 $p = \norm{A\ket{\psi}}^2/\alpha^2$); 失败则丢弃重试, 或用 Module2 的振幅放大提升成功概率.
...
\begin{Note}[与 GSLW 完整构造的关系]
    上述电路是 Gily\'en--Su--Low--Wiebe(GSLW) $s$-稀疏 block-encoding 的简化版: 完整构造还需
    同时访问\textbf{行索引 oracle} $O_r$ 与\textbf{列索引 oracle} $O_c$, 并在行、列寄存器间做一次
    SWAP 与 uncomputation(见 GSLW 引理 48). 本节的列 oracle 构造对每列非零元个数恰为 $s$、
    且列 oracle 足以确定全部稀疏结构的矩阵成立; 对一般 $s$-稀疏矩阵应以 GSLW 的完整电路为准.
\end{Note}
...
block-encoding 的作用转化为依赖于 $x_j$ 的二维旋转. 本讲义统一约定角度 $\theta_j$ 使
$\cos\theta_j = \sigma_j/\alpha$(与 5.5 节 Hermitian 情形的 $\cos\theta_i = \lambda_i$ 一致):
\begin{equation}
...
第二项系数由 $\norm{U_A\ket{0^a}\ket{v_i}} = 1$ 确定. 又因为 $U_A$ 厄米且 $U_A^2 = I$,
由 $U_A^2\ket{0^a}\ket{v_i} = \ket{0^a}\ket{v_i}$ 解出
\begin{equation}
    U_A\ket{\Phi_i} = \sqrt{1-\lambda_i^2}\,\ket{0^a}\ket{v_i} - \lambda_i\ket{\Phi_i} \in H_i,
\end{equation}
其中 $H_i := \mathrm{span}\{\ket{0^a}\ket{v_i}, \ket{\Phi_i}\}$(也可直接看到
$\bra{0^a}\bra{v_i}U_A\ket{\Phi_i} = \big(\bra{\Phi_i}U_A\ket{0^a}\ket{v_i}\big)^* = \sqrt{1-\lambda_i^2} \ne 0$).
故 $H_i$ 是 $U_A$ 的\textbf{二维不变子空间}(详细列块见下一步). 整个高维问题因此被分块对角化为 $N$ 个互不相干的
...
\noindent 电路只需交替施加 $U_A$, $U_A^\dagger$ 与 $Z$(一次 walk 含两个来回):
\begin{equation*}
\Qcircuit @C=1em @R=.9em {
    \lstick{\ket{0^a}} & \multigate{1}{U_A} & \gate{Z} & \multigate{1}{U_A^\dagger} & \gate{Z} & \multigate{1}{U_A} & \gate{Z} & \multigate{1}{U_A^\dagger} & \gate{Z} & \qw \\
    \lstick{\ket{\psi}} & \ghost{U_A} & \qw & \ghost{U_A^\dagger} & \qw & \ghost{U_A} & \qw & \ghost{U_A^\dagger} & \qw & \qw
}
\end{equation*}
其中 $Z = 2\Pi - I$ 是辅助比特为 $\ket{0^a}$ 时的相位翻转; Hermitian 情形 $U_A^\dagger = U_A$,
电路退化为 $U_A$ 与 $Z$ 交替.
...
$c_n$ 进行\textbf{相干组合(coherent combination)}, 而各个 $O^k$ 之间无法直接以任意复系数叠加.
一种朴素做法是再次使用 LCU(§3)去组合这些 $T_n$, 但这往往需要额外辅助寄存器与振幅放大.
\textbf{量子信号处理(quantum signal processing, QSP)}提供了更直接的方法: 在迭代算子 $O(x)$
之间插入一系列与信号 $x$ 无关的可调相位 $e^{i\phi_k Z}$, 使各阶 $T_n$ 通过\textbf{量子干涉}
以受控系数在同一个酉电路中相干叠加, 而不是在事后组合 $O^k$. 因此
\textbf{Qubitization + Chebyshev 结构本身还不是 QSP, 而是 QSP 的数学基础}:
qubitization 把谱变量 $x = \lambda/\alpha$ 变成二维旋转($x = \cos\theta$, §5.5),
Chebyshev 多项式说明旋转的幂 $O^k$ 给出 $T_k(x)$($\cos(k\theta) = T_k(x)$),
而 QSP 再用相位把单一的 Chebyshev 模式推广为任意可编程多项式 $P(x)$.
QSP 所回答的问题是: 给定目标多项式 $P(x)$, 如何把它编译成一组 quantum phases?
实际中分成两个子问题: \textbf{approximation problem}(找满足 QSP 条件的 $P(x) \approx f(x)$)与
\textbf{phase synthesis problem}(把 $P$ 编译成相位序列 $\vec\phi$);
可实现的 $P$ 需满足的次数、奇偶性与有界性条件, 将在下一节的表示定理中精确刻画.
...
\begin{equation}
    U_{\vec\phi}(x) := e^{i\phi_0 Z}\, O(x)\, e^{i\phi_1 Z}\, O(x) \cdots e^{i\phi_{d-1} Z}\, O(x)\, e^{i\phi_d Z},
\end{equation}
其中 $\vec\phi = (\phi_0, \dots, \phi_d) \in \mathbb{R}^{d+1}$ 是\textbf{相位因子(phase factors)};
序列共调用 $d$ 次 $O(x)$, 相位旋转 $S(\phi_k) := e^{i\phi_k Z} = \mathrm{diag}(e^{i\phi_k}, e^{-i\phi_k})$
本身不依赖于信号 $x$. 电路即单比特信号线路上相位旋转与 signal unitary 交替($d = 3$ 示意):
\begin{equation*}
\Qcircuit @C=1em @R=.9em {
    \lstick{\ket{\psi}} & \gate{S(\phi_0)} & \gate{O(x)} & \gate{S(\phi_1)} & \gate{O(x)} & \gate{S(\phi_2)} & \gate{O(x)} & \gate{S(\phi_3)} & \rstick{\ket{\psi'}} \qw
}
\end{equation*}
其中 $\bra{0}U_{\vec\phi}(x)\ket{0} = P(x)$ 就是我们要的多项式; 注意 $\phi_k$ 是经典可调参数,
编译一次后以单比特相位门的形式固定进电路.
...
\begin{Note}[signal unitary 的约定]
    ...
    例如在 $W$ 约定下, 表示定理取形
    \begin{equation}
        U_{\vec\phi}(x) = \begin{pmatrix} P(x) & i\sqrt{1-x^2}\,Q(x) \\
...
\begin{Thm}[QSP 表示定理]
    对任意 $\vec\phi \in \mathbb{R}^{d+1}$, 存在多项式 $P, Q \in \mathbb{C}[x]$ 使得
    \begin{equation}
        U_{\vec\phi}(x) = \begin{pmatrix}
            P(x) & -Q(x)\sqrt{1-x^2} \\
    ...
    反之, 任何满足这三条条件的 $P, Q$ 都可以由某个相位序列 $\vec\phi$ 表示.
\end{Thm}

\begin{Proof}[表示的正方向(归纳)]
    记 $U_{\vec\phi}^{(d)} := S(\phi_0)\,O(x)\,S(\phi_1)\,O(x)\cdots O(x)\,S(\phi_d)$($d$ 次 $O$).
    $d = 0$ 时 $U = S(\phi_0)$ 给出 $P = e^{i\phi_0}$, $Q = 0$. 归纳步: 设
    $U_{\vec\phi}^{(d-1)} = S(\phi_1)\,O(x)\,S(\phi_2)\cdots O(x)\,S(\phi_d)$(相位
    $\phi_1,\dots,\phi_d$, $d-1$ 次 $O$)的标准形式为 $(P', Q')$, 则
    $U_{\vec\phi}^{(d)} = S(\phi_0)\,O(x)\,U_{\vec\phi}^{(d-1)}$. 记 $s = \sqrt{1-x^2}$,
    直接计算:
    \begin{equation}
        S(\phi_0)\,O(x)\begin{pmatrix} P' & -sQ' \\ s\overline{Q'} & \overline{P'} \end{pmatrix}
        = \begin{pmatrix}
            e^{i\phi_0}\big(xP' - (1-x^2)\overline{Q'}\big) & -s\,e^{i\phi_0}\big(xQ' + \overline{P'}\big) \\
            s\,e^{-i\phi_0}\big(x\overline{Q'} + P'\big) & e^{-i\phi_0}\big(x\overline{P'} - (1-x^2)Q'\big)
        \end{pmatrix},
    \end{equation}
    故新 $P = e^{i\phi_0}\big(xP' - (1-x^2)\overline{Q'}\big)$,
    新 $Q = e^{i\phi_0}\big(xQ' + \overline{P'}\big)$.
    由 $\deg P' \le d-1$, $\deg Q' \le d-2$ 得 $\deg P \le d$, $\deg Q \le d-1$;
    $P'$ 的奇偶性为 $(d-1)\bmod 2$, $Q'$ 的奇偶性为 $(d-2)\bmod 2$, 代入上式即得
    $P$ 的奇偶性为 $d\bmod 2$, $Q$ 的奇偶性为 $(d-1)\bmod 2$ —— 归纳完成.
    条件 (3) 是酉性 $U^\dagger U = I$ 的直接推论(第一行的两个元素模方和为 $1$).
\end{Proof}
...
\begin{Note}[次数、奇偶性与酉性约束]
    ...
    $U_{\vec\phi}(x)^\dagger U_{\vec\phi}(x) = I$ 得
    \begin{equation}
        |P(x)|^2 + (1-x^2)|Q(x)|^2 = 1,
        \qquad |P(x)| \le 1 \quad (x \in [-1,1]),
        \label{eq:qsp_unitary}
    \end{equation}
    它来自量子力学最基本的要求: 最终电路必须保持概率归一化. 因此 QSP 不能直接实现
    无界多项式: 若目标函数超过 $1$, 需先归一化 $\widetilde f(x) = f(x)/\beta$ 使
    $|\widetilde f(x)| \le 1$.

    \textbf{端点约束(重要).} 在式 \eqref{eq:qsp_unitary} 中令 $x = \pm 1$,
    $1-x^2 = 0$, 立得
    \begin{equation}
        |P(\pm 1)| = 1,
    \end{equation}
    即 QSP 可实现的任何多项式在端点 $x = \pm 1$ 处模长恒为 $1$, 且与次数 $d$ 无关.
    因此对\textbf{实值}目标函数 $f$ 做 QSP 逼近时, 若 $f(\pm 1) \neq \pm 1$, 端点处的
    最小可能误差为 $\min\big\{|1-f(\pm1)|,\ |1+f(\pm1)|\big\}$, 无法通过增大次数消除.
    例如逼近 $f(x) = \cos(tx)$ 时 $\cos(\pm t) = \cos t \neq \pm 1$(除非 $t = \pi$),
    端点误差至少为 $|1 - \cos t|$; 实践中应选择满足 $|f(\pm 1)| = 1$ 的目标
    (如 $\cos(\pi x)$, 或复数目标 $e^{-itx}$ 的实部), 或把端点误差计入逼近预算.
\end{Note}
...
\begin{Note}[实多项式版本的充分条件]
    ...
    则存在相位序列 $\vec\phi$ 使 $U_{\vec\phi}$ 的 $(1,1)$ 元素恰为 $P_{\mathrm{Re}}(x)$.
    相比复数版本, 条件 (2) 只需对任意多项式做归一化缩放即可满足.
    补多项式 $Q$ 的存在性由如下事实保证: 端点约束 $|P_{\mathrm{Re}}(\pm1)|=1$ 使
    $1 - P_{\mathrm{Re}}(x)^2$ 在 $x=\pm1$ 处为零、从而可被 $(1-x^2)$ 整除, 且
    $1-P_{\mathrm{Re}}(x)^2\ge0$($x\in[-1,1]$); 由 \textbf{Fej\'er--Riesz 型分解}
    (把 $R(\cos\theta) := \frac{1-P_{\mathrm{Re}}(\cos\theta)^2}{1-\cos^2\theta}$ 视为单位圆上的
    非负三角多项式)存在满足次数与奇偶性条件的复多项式 $Q$ 使
    $|Q(x)|^2 = R(x)$, 即 $|P_{\mathrm{Re}}|^2 + (1-x^2)|Q|^2 = 1$ —— polynomial completion 自动完成.
...
\begin{Eg}[二阶 QSP: $d = 2$]
    ...
    phase parameters $\to$ polynomial coefficients. 这也可以看作表示定理归纳
    (左乘 $S(\phi_0)O(x)$ 的递推, 见上面证明)在 $d=2$ 时的显式验证:
    取 $U^{(1)} = S(\phi_1)O(x)S(\phi_2)$ 的 $P' = e^{i(\phi_1+\phi_2)}x$,
    $Q' = e^{i(\phi_1-\phi_2)}$ 代入 $P = e^{i\phi_0}\big(xP' - (1-x^2)\overline{Q'}\big)$
    即得上式.
\end{Eg}

\subsection{量子本征值变换 (QET)}

把标量 $x$ 换成厄米矩阵 $A$, 即可把 QSP"提升"为矩阵版本 —— 这正是 qubitization 的意义.
回顾 5.5 节的结论: 对 Hermitian block-encoding $U_A$, 每个本征态 $\ket{\lambda_j}$ 对应一个
二维不变子空间, $U_A$ 在其中是反射、$U_A Z$ 是旋转角 $\theta_j$($\cos\theta_j = \lambda_j/\alpha$)
的旋转. 因此只要把标量 QSP 序列中的 signal unitary $O(x)$ 替换为 $U_A Z$, 同一个相位序列
$S(\phi_0), \dots, S(\phi_d)$ 就会在所有本征方向的二维子空间中\textbf{同步}产生同一个标量变换
$x_j \mapsto P(x_j)$ —— 相位调制作用在辅助比特上, 与矩阵方向无关. 由谱分解
$A = \sum_j \lambda_j \ket{\lambda_j}\bra{\lambda_j}$, 这等价于对每个本征值同时施加 $P$:
\begin{equation}
    P(A/\alpha) = \sum_j P(\lambda_j/\alpha)\,\ket{\lambda_j}\bra{\lambda_j},
\end{equation}
这是从标量多项式变换进入矩阵多项式变换的关键一步. 注意其中的 $P$ 必须满足 QSP 表示定理的
次数、奇偶性与有界性条件; 下面的 QET 定理给出这一提升的严格表述.

\begin{Thm}[QET: Hermitian block-encoding]
    设 $U_A \in \mathrm{HBE}_{1,a}(A)$. 对任意相位序列 $\vec\phi \in \mathbb{R}^{d+1}$,
    \begin{equation}
        U := S(\phi_0)\,(U_A Z)\, S(\phi_1)\,(U_A Z)\, \cdots\, (U_A Z)\, S(\phi_d),
        \qquad S(\phi_k) := e^{i\phi_k Z},
    \end{equation}
    即相位旋转与 \textbf{qubitization 迭代算子 $O = U_A Z$} 交替作用 $d$ 次, 则
    $U = \begin{pmatrix} P(A) & \ast \\ \ast & \ast \end{pmatrix}$,
    即 $U \in \mathrm{BE}_{1,a}(P(A))$, 其中 $P$ 满足 QSP 定理的三条条件.
    电路结构: 交替施加 $U_A$ 与 $Z$($Z = 2\Pi - I$ 为辅助比特上的受控相位, 见 5.5 节),
    共 $O(d)$ 次 $U_A$ 调用与 $O(d)$ 次单比特相位旋转.
\end{Thm}

\begin{Proof}
    由 5.5 节的 qubitization, 每个本征方向 $i$ 对应二维不变子空间 $H_i$,
    $U_A Z$ 在其上是旋转角 $\theta_i$($\cos\theta_i = \lambda_i$)的旋转:
    \begin{equation}
        [U_A Z]_{B_i} = \begin{pmatrix} \lambda_i & -\sqrt{1-\lambda_i^2} \\
            \sqrt{1-\lambda_i^2} & \lambda_i \end{pmatrix}
        = O(\lambda_i),
    \end{equation}
    恰为标量 QSP 的 signal unitary $O(\lambda_i) = U_A(\lambda_i)Z$(§6.1)的形式.
    因此标量 QSP 表示定理在\textbf{每个}二维子空间上逐块成立: 同一个相位序列在所有本征方向
    上同步实现 $x_i \mapsto P(x_i)$, 投影回成功子空间即得
    $\big(\bra{0^a}\otimes I\big)U\big(\ket{0^a}\otimes I\big) = P(A)$.
\end{Proof}

电路示意($d = 3$; 每个 step 为 qubitization 迭代算子 $U_A Z$, 虚线辅助比特 $\ket{0^a}$ 标志成功子空间):
\begin{equation*}
\Qcircuit @C=1em @R=.9em {
    \lstick{\ket{0^a}} & \gate{S(\phi_0)} & \multigate{1}{U_A Z} & \gate{S(\phi_1)} & \multigate{1}{U_A Z} & \gate{S(\phi_2)} & \multigate{1}{U_A Z} & \gate{S(\phi_3)} & \rstick{\ket{0^a}} \qw \\
    \lstick{\ket{\psi}} & \qw & \ghost{U_A Z} & \qw & \ghost{U_A Z} & \qw & \ghost{U_A Z} & \qw & \rstick{P(A)\ket{\psi}} \qw
}
\end{equation*}
...
\begin{Eg}[Chebyshev 多项式与相位因子]
    ...
    $T_d(A)$ 的 block-encoding —— 这正是 QET 电路以 $U_A Z$ 为 step 的原因.
    更一般地, 由于 $Z$ 与 $S(\phi_k) = e^{i\phi_k Z}$ 对易, 每个 step 的 $Z$ 可与相邻的
    相位旋转合并($S(\phi_k)Z$ 仍是辅助比特上的对角相位门), 电路实现时只需 $U_A$ 与
    单比特相位交替, 无需单独的 $Z$ 门.
\end{Eg}
...
QSVT 把这一标量多项式变换提升到一般矩阵的\textbf{奇异值}上. 与上一章 QET 只适用于厄米矩阵
(特征值变换)不同, 一般矩阵没有特征值分解, 但 block-encoding 为其\textbf{每一个奇异值}
都提供一个二维信号子空间(§7.3), 于是同一个标量 QSP 相位序列可以在所有这些子空间中同时
起作用 —— "QSP 表示定理 + 奇异值分解 + block-encoding"三者合流, 正是 QSVT 的机制
(§7.8 给出完整论证). 形式地, 设
\begin{equation}
...
\begin{Note}[记号警示: 与普通矩阵函数的区别]
    这里的 $f(A) := U f(\Sigma) V^\dagger$ 是\textbf{奇异值意义}下的(对称)变换, 与普通矩阵
    多项式 $P(A) = \sum_k c_k A^k$(幂级数/谱定义, 见 Module1)在一般非厄米矩阵上\textbf{不同}:
    例如 $P(x) = x^2$ 时 $P(A) = A^2 = U\Sigma V^\dagger U\Sigma V^\dagger \ne U\Sigma^2 V^\dagger$
    (因为 $V^\dagger U \ne I$). 只有当 $A$ 厄米(从而左右奇异向量一致)时二者才相同;
    7.7 节将专门澄清这一区别. 本讲义中 $f^{(\mathrm{SV})}$、$f^\triangleleft$、$f^\triangleright$
    均指奇异值变换, 而非普通矩阵函数.
\end{Note}
...
($A\ket{v_i} = \sigma_i\ket{u_i}$, $A^\dagger\ket{u_i} = \sigma_i\ket{v_i}$),
...
\begin{Eg}[Hermitian 情形]
    ...
    联系. 对\textbf{偶}多项式 $P$: $P^\triangleleft(A) = P^\triangleright(A) = P(A)$; 对\textbf{奇}多项式 $P$:
    $P^\triangleleft(A) = P(|A|) = \sum_i \mathrm{sgn}(\lambda_i)\,P(\lambda_i)\ket{v_i}\bra{v_i}$，而 QET 给出的特征值函数为 $P(A) = \sum_i P(\lambda_i)\ket{v_i}\bra{v_i}$.
...
($U_A^\dagger = U_A$, 只需 $U_A$)的本质区别. 从 7.4 节的 walk 结构看, 每个"来回"
$(U_A Z U_A^\dagger Z)$ 在奇异值子空间内旋转 $2\theta_j$、给出 $T_2(x_j)$(即两次 $U_A$ 调用
贡献两个多项式度); QSP 相位序列则进一步在 $U_A$ 与 $U_A^\dagger$ 之间插入可调相位,
实现一般的 $P(x_j)$.
...
$e^{i\phi_k(2\widetilde\Pi - I)}$ 交替组合而成: 对标准方阵 block-encoding
($\Pi = \widetilde\Pi = \ket{0^a}\bra{0^a}\otimes I$), 这里的 $2\Pi - I$ 正是 5.5 节的
$Z$ 门(辅助比特为 $\ket{0^a}$ 时翻转相位), 电路即"$U_A$/$U_A^\dagger$ 与 $Z$ 交替",
与 QET(§6.4)和 walk(§7.4)的记号完全一致. 在每个奇异值子空间中, 该高维电路
等价于一个普通的 QSP sequence, 因此最终实现 $x_j \mapsto P(x_j)$; 相位设计问题
本质上仍然是 QSP phase synthesis. 电路示意($d = 3$, 奇多项式, $U_A$ 与 $U_A^\dagger$ 交替):
\begin{equation*}
\Qcircuit @C=1em @R=.9em {
    \lstick{\ket{0^a}} & \gate{S(\phi_0)} & \multigate{1}{U_A} & \gate{S(\phi_1)} & \multigate{1}{U_A^\dagger} & \gate{S(\phi_2)} & \multigate{1}{U_A} & \gate{S(\phi_3)} & \rstick{\ket{0^a}} \qw \\
    \lstick{\ket{\psi}} & \qw & \ghost{U_A} & \qw & \ghost{U_A^\dagger} & \qw & \ghost{U_A} & \qw & \rstick{P(A)\ket{\psi}} \qw
}
\end{equation*}
偶多项式时以 $U_A^\dagger$ 结尾(输出在右奇异子空间, 即 $P^\triangleleft(A)$);
相位旋转 $S(\phi_k) = e^{i\phi_k Z}$ 与 QET 相同, 故"相位综合"问题与 QSP 完全一致.
...
\begin{Def}[厄米膨胀]
    ...
    \begin{equation}
        ...
        \qquad
        \bar{A}\ket{z_i^\pm} = \pm \sigma_i \ket{z_i^\pm},
    \end{equation}
\end{Def}
...
\begin{Prop}[QET(Ā) 蕴含 QSVT(A)]
    ...
    在左上块内信号比特分别只有 $\ket{0}$/$\ket{1}$ 分量, 因此测量信号比特的结果是
    \textbf{确定性}的: 偶(奇)函数情形无需对信号比特做后选择来区分成功分支. 需要说明的是,
    完整的辅助寄存器($\ket{0^a}\ket{0}$)测量概率为
    $\norm{f_{\mathrm{even}}^\triangleleft(A)\ket{\psi}}^2$(偶函数情形), 当 $|f(\sigma_j)|=1$
    (如酉变换 $e^{-itx}$ 的精确多项式实现)时等于 $1$.
\end{Prop}
...
是与 $(A^{-1})^\dagger$ 成比例的变换; 为制备 $A^{-1}\ket{b}$, 应对 $A^\dagger$ 的 block-encoding(即
...
\begin{Thm}[基于 QET/QSVT 的哈密顿量模拟]
    ...
    \end{equation}
    其中电路深度按 $U_H$ 的调用次数计. 该复杂度匹配哈密顿量模拟的查询下界
...
\begin{equation}
    \Big|p(x) - \frac{1}{\kappa x}\Big| \le \varepsilon, \qquad x \in [-1, -1/\kappa] \cup [1/\kappa, 1],
\end{equation}
且 $|p(x)| \le 1$($x \in [-1,1]$，因 $1/(\kappa x)$ 在 $[1/\kappa,1]$ 上取值 $\le 1$). 这样的多项式存在且
...
\begin{Thm}[\textbf{量子线性系统问题(QLSP)} 的\textbf{查询复杂度(query complexity)}]
    ...
    (此时 $\zeta = \Omega(\kappa)$、成功概率 $\Theta(\zeta^2/\kappa^2)=\Omega(1)$，无需振幅放大), 则降为 $O(\kappa\log(\kappa/\varepsilon))$;
...
\begin{Note}[Grover 搜索与固定点振幅放大的重访]
    在 QSVT 视角下, Grover 迭代正是对奇异值区间 $[1/\sqrt{N}, 1]$ 的奇多项式变换
    ($p(x) \approx 1$ 于其上); 单比特 QSP 电路统一了
...
\begin{Note}[本模块的主要文献]
    ...
    \begin{itemize}
        ...
        (block-encoding 小演算、QSVT 的逆滤波器/热半群/振荡传播子应用、
        差分算子移位 block-encoding 等), arXiv:2607.09639 (2026).
    \end{itemize}

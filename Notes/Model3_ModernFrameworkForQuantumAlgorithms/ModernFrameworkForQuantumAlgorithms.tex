...
构造\textbf{反射算子(reflection operator)}, 其实现依赖具体问题. 本模块介绍\textbf{现代统一框架}, 它由三个层层递进的概念组成:
...
\begin{enumerate}
    ...
    "反射"转化为"旋转", 从而一步实现\textbf{Chebyshev 多项式(Chebyshev polynomials)} $T_k(A)$;
...
\begin{Note}[路线图]
    ...
    Block-encoding/\textbf{线性组合酉(LCU)}(S5)、QSP/Qubitization(S6)、QSVT(S7).
    在下一模块中, \textbf{HHL(Harrow--Hassidim--Lloyd)}(S8)与薛定谔化(S9)将直接建立在这些构件之上;
    而 \textbf{Grover 搜索(Grover search)}、QPE、振幅放大等 Module2 的原语都可以在这一框架中被重新推导并推广.
\end{Note}
...
设 $U_A \in \mathrm{HBE}_{1,m}(A)$, $A$ 的\textbf{谱分解(spectral decomposition)}为 $A = \sum_i \lambda_i \ket{v_i}\bra{v_i}$.
对每个本征态 $\ket{v_i}$,
\begin{equation}
...
\begin{Note}[Oblivious amplitude amplification]
    ...
    因此可以用只检查辅助寄存器的\textbf{oblivious 振幅放大(oblivious amplitude amplification, OAA)}:
    \begin{equation}

...
\begin{Note}[带源项: 增广齐次化]
    ...
    源项使扩展后的 $H_1$ 不再保持半负定: 由 Weyl 扰动定理,
    \[
...
薛定谔化不是处理非酉动力学唯一的方法. 从"把非酉传播子放进更大酉结构"的抽象视角看,
四种常用路线可以放在同一个模板下:
\begin{equation}
    M(T) = e^{TA} \;\approx\;
...
\begin{tabular}{llll}
    ...
    一般线性非酉 ODE/PDE \\
    LCHS & 对 $w$ 做连续 Fourier 后离散 $\xi$: $u(T) \approx \sum_m c_m U_{\xi_m}(T)u_0$ &
    酉演化的 LCU &
    积分表示清楚的系统 \\
    Transmutation & $A = G^\dagger G$, 热半群写成波传播子的 Gaussian 平均 &
    LCU + Hermitian dilation $D(G)$ &
    热方程、扩散型自伴问题 \\
    Moment-matching dilation & 构造辅助动力学 $F$ 使 $\bra{\ell}F^q\ket{r} = 1$ &
